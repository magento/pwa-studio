...
class ProxyResolver extends AbstractResolver {
    ...
    async resolve(definition) {
        if (!definition.target) {
            throw new Error(
                `'target' URL argument is required: ${JSON.stringify(definition)}`
            );
        }
        const toResolve = [
        ...
        if (typeof target !== 'string') {
            throw new Error(`'target' argument to ProxyResolver must be a string URL, but was a: ${typeof target}`);
        }
