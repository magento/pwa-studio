...
export const useQuantityStepper = props => {
    const { initialValue, min, onChange, fieldName, isQtyChange } = props;
    ...
    const handleBlur = useCallback(
        e => {
            ...
            if (e) {
                debouncedOnChange(Number(e.target.value));
            }
            if (typeof quantity === 'number' && quantity != prevQuantity) {
                debouncedOnChange(quantity);
            }
        },
        [debouncedOnChange, prevQuantity, quantity, isQtyChange]
    );
