...
import { outOfStockData, mockStockData, productWithThreeOptions } from "../mockData";
...
test('shows `out of stock` message when current configuration is unavailable.', async () => {
    ...
    const secondaryOption = {
        color: wrapper.instance().props.product.configurable_options[1].values[1]
    };
...
test('getCurrentConfiguration returns correct SKU', async () => {
    ...
    const currentConfiguration = wrapper.instance().getCurrentConfiguration(product, newState);
