...
describe('supportedArguments', () => {
    test('it supports clients, filesGlob, and project', () => {
        const keys = Object.keys(plugin.supportedArguments);

        expect(keys).toHaveLength(3);
        expect(keys).toContain('clients');
        expect(keys).toContain('filesGlob');
        expect(keys).toContain('project');
    });

    test('the clients argument has apollo as its default', () => {
        const { clients } = plugin.supportedArguments;

        expect(clients).not.toBeUndefined();
        expect(clients.default).not.toBeUndefined();
        expect(clients.default).toHaveLength(1);
        expect(clients.default).toContain('apollo');
    });
});
...
describe('handler', () => {
    ...
    test('it logs an appropriate message when there are no errors', async () => {
        ...
        expect(mockConsoleLog).toHaveBeenCalledWith('All queries are valid.');
