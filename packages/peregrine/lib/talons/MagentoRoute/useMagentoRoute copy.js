...
export const useMagentoRoute = (props = {}) => {
    ...
    const queryResult = useQuery(resolveUrlQuery, {
        ...
        skip: !(initialized.current || !getInlinedPageData()),
        onCompleted: () => (fetchedPathname.current = pathname)
    });
    ...
    const component = IS_BROWSER
        ? componentMap.get(pathname)
        : !type || isEmpty
        ? null
        : createComponentData(route);
    ...
    useEffect(() => {
        ...
        (async () => {
            const componentData = createComponentData(route);
            if (componentData) {
                setComponent(pathname, componentData);
            }
        })();
    ...
    useEffect(() => {
        (async () => {
            ...
            if (inlinedData) {
                try {
                    setComponent(pathname, createComponentData(inlinedData));
                } catch (error) {
...
const createComponentData = route => {
    ...
    try {
        return {
            component: getRootComponent(type),
